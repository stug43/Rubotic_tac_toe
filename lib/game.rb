...
class Game
	...
	def is_free?(choice)
		return !@data.busy_cells[choice.to_sym]
	end

	def new_tour
		self.def_current_player(@compt_tour)
		choice = @view.new_tour(@current_player)
		self.actualize_database(choice)
	...
	def display_array(array_1 = @data.hash_pl1.values, array_2 = @data.hash_pl2.values)
		...
		display_array.map!.with_index do |k, index|
	...
	def is_full?(hash)
		!hash.values.include?(false)
	end

	def perform
		loop do
			self.new_tour()	
			if self.has_won?(@data.hash_pl1.values)||self.has_won?(@data.hash_pl2.values)
				@view.won(@current_player)
				break
			elsif self.is_full?(@data.busy_cells)
				@view.is_full
				break
			end
			@compt_tour += 1
